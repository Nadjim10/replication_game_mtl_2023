...
### Identify unique parties being evaluated
...
nice_normality(dta_unique,
               variable = "to_pfeml",
               histogram = TRUE,
               #group = "country",
               title = "Proportion of Women MPs")

nice_density(dta_unique,
             variable = "to_pfeml",
             histogram = TRUE,
             # group = "country",
             title = "Proportion of Women MPs")
...
dta_unique |> 
  count(to_pfeml, sort = TRUE, name = "Frequency") |> 
  rename(`Proportion of Women MPs` = to_pfeml) |> 
  nice_scatter(predictor = "Proportion of Women MPs",
               response = "Frequency")

# increase of female MP by year, party, family

# dygraph, by year and party family
...
### With clustered SEs
stargazer(type = "text", tableS2.1, tableS2.2, 
          add.lines = list(c("Country-Year Fixed Effects?", "Yes"), c("Country-Level Clustered SEs?", "Yes")),
...
### Time series plot of number of female MPs, by political party family and year

# Political family ####

q <- dta %>%
  group_by(year, to_parfam) %>% 
  select(year, to_parfam, to_pfeml) %>% 
  summarize(to_pfeml = mean(to_pfeml)) %>% 
  mutate(year = as.Date(ISOdate(year, 1, 1)),
         year = as.Date(year, "%Y")) %>% 
  pivot_wider(names_from = to_parfam, values_from = to_pfeml) %>% 
  as.xts
...
q <- dta %>%
  ...
  select(year, country, to_pfeml) %>% 
  summarize(to_pfeml = mean(to_pfeml)) %>% 
  mutate(year = as.Date(ISOdate(year, 1, 1)),
         year = as.Date(year, "%Y")) %>% 
  pivot_wider(names_from = country, values_from = to_pfeml) %>% 
  as.xts
...
q <- dta %>%
  mutate(left_right = case_when(
    to_parfam == 10 ~ "Left", # Green
    to_parfam == 20 ~ "Left", # Rad Left/Socialist
    to_parfam == 40 ~ "Left", # Liberal
    to_parfam == 30 ~ "Left", # Social Democratic/Center-Left
    to_parfam == 50 ~ "Right", # Christian Democratic
    to_parfam == 60 ~ "Right", # Conservative
    to_parfam == 70 ~ "Right" # Radical Right
    )) %>% 
  group_by(year, left_right) %>% 
  select(year, left_right, to_pfeml) %>% 
  summarize(to_pfeml = mean(to_pfeml)) %>% 
  mutate(year = as.Date(ISOdate(year, 1, 1)),
         year = as.Date(year, "%Y")) %>% 
  pivot_wider(names_from = left_right, values_from = to_pfeml) %>% 
  as.xts
