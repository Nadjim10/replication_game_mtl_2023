...
table(updated_data$party_like, useNA = "always")

updated_data <- updated_data %>% 
  mutate(dyad = paste(to_mp_number, "to", from_mp_number, sep = "_"))
...
# Other
multilevel_data <- multilevel_data |> 
  select(year, country, rile_distance_s, prior_coalition, 
         prior_opposition, econ_distance_s, society_distance_s,
...
ID <- multilevel_data$ID
thermometer_score <- multilevel_data$thermometer_score
cntryyr <- multilevel_data$cntryyr
dyad <- multilevel_data$dyad 
prior_opposition <- multilevel_data$prior_opposition 
rile_distance_s <- multilevel_data$rile_distance_s

test <- multilevel_data %>% group_by(dyad, year) %>% summarize(party_like = mean(thermometer_score))
...
test <- multilevel_data %>% 
  select(year, country, rile_distance_s, prior_coalition, 
         prior_opposition, econ_distance_s, society_distance_s,
         cntryyr, to_pfeml, to_prior_seats, to_mp_number, dyad) %>% 
  # select(year, to_pfeml, gender, thermometer_score) %>%  #from_pfeml
  group_by(dyad, year) %>% 
  summarize(party_like = mean(thermometer_score),
            party_dislike = 10 - mean(thermometer_score)) %>% 
...
# Variables
set.seed(100)
multilevel_data <- missForest(multilevel_data, verbose = TRUE, parallelize = "variables")

indiv_data <- multilevel_data$ximp

indiv_data$ID <- ID
indiv_data$thermometer_score <- thermometer_score
indiv_data$cntryyr <- cntryyr
indiv_data$dyad <- dyad
indiv_data$prior_opposition <- prior_opposition
indiv_data$rile_distance_s <- rile_distance_s
indiv_data$prior_coalition <- prior_coalition

lapply(list(indiv_data$prior_coalition,
            indiv_data$thermometer_score,
            indiv_data$cntryyr,
            indiv_data$dyad,
            indiv_data$prior_opposition,
            indiv_data$rile_distance_s,
            indiv_data$prior_coalition), 
